...
function OperacjeLista({rows,allRows,me,load,kontrahenci=[],opakowania=[],magazyny=[],notify,selectable=false,selected={},toggleSelect=()=>{}}){
 ...
 const startEdit=(o)=>{
  ...
  setEdit({
   ...
   pozycje:(doc.pozycje&&doc.pozycje.length?doc.pozycje:[{id:doc.id,opakowanie:doc.opakowanie,ilosc:doc.ilosc}]).map(x=>({uid:Date.now()+Math.random(),id:x.id,opakowanie:x.opakowanie||'',ilosc:x.ilosc||1})),
   usunietePozycje:[]
  });
  setTimeout(()=>{document.getElementById('editDocBox')?.scrollIntoView({behavior:'smooth',block:'start'});},80);
 };
