...
function App(){
 ...
 const load=async()=>{ if(!supabase){setErr('Brak zmiennych VITE_SUPABASE_URL lub VITE_SUPABASE_ANON_KEY w Vercel.'); setReady(true); return}
  try{
   ...
   setKontrahenci(k.data||[]); setOpakowania(o.data||[]); setMagazyny(m.data||[]); setUsers(u.data||[]); setOperacje(op.data||[]); setUsuniete(del.data||[]); setReady(true);
 ...
 return <Shell>
   <header className="top"><div><b>Agromarbanka</b><span>Online Supabase</span></div><button onClick={()=>setMe(null)}><LogOut size={16}/> Wyloguj</button></header>
...
function Login({users,setMe}){const [tel,setTel]=useState(''),[pin,setPin]=useState(''),[msg,setMsg]=useState(''); const login=()=>{const u=users.find(x=>x.telefon===tel.trim()&&x.pin===pin.trim()&&x.aktywny!==false); if(u) setMe(u); else setMsg('Nieprawidłowy telefon lub PIN')}; return <Shell><div className="login"><h2>Panel logowania</h2><input placeholder="Telefon" value={tel} onChange={e=>setTel(e.target.value)}/><input placeholder="PIN" value={pin} onChange={e=>setPin(e.target.value)} type="password"/><button className="primary" onClick={login}>Zaloguj</button><small>Demo admin: 500000000 / 1111</small>{msg&&<p className="error">{msg}</p>}</div></Shell>}
function SearchSelect({label,items,value,setValue,field='nazwa'}){
...
function Operacje(p){const mags=p.me.rola==='kierowca'&&p.me.magazyn?p.magazyny.filter(m=>m.nazwa===p.me.magazyn):p.magazyny; const [kon,setKon]=useState(''),[opa,setOpa]=useState(''),[mag,setMag]=useState(mags[0]?.nazwa||''),[ilosc,setIlosc]=useState(1),[data,setData]=useState(today()),[podpis,setPodpis]=useState(''); useEffect(()=>{if(!mag&&mags[0])setMag(mags[0].nazwa)},[mags]);
 const save=async(typ)=>{if(!kon||!opa||!mag||!ilosc)return alert('Uzupełnij kontrahenta, opakowanie, magazyn i ilość'); const r=await supabase.from('operacje').insert({kontrahent:kon,opakowanie:opa,magazyn:mag,typ,ilosc:Number(ilosc),data_operacji:data,podpis,uzytkownik:p.me.imie}); if(r.error) return alert(r.error.message); setIlosc(1); setData(today()); setPodpis(''); p.load();};
 return <div className="grid"><section className="card"><h2><Package/> Operacja</h2><SearchSelect label="Kontrahent" items={p.kontrahenci.filter(x=>x.aktywny!==false)} value={kon} setValue={setKon}/><SearchSelect label="Opakowanie" items={p.opakowania.filter(x=>x.aktywne!==false)} value={opa} setValue={setOpa}/><div className="group"><label>Magazyn</label><select value={mag} onChange={e=>setMag(e.target.value)}>{mags.filter(x=>x.aktywny!==false&&!x.ukryty).map(m=><option key={m.id}>{m.nazwa}</option>)}</select></div><div className="row"><div><label>Ilość</label><input type="number" value={ilosc} onChange={e=>setIlosc(e.target.value)}/></div><div><label>Data operacji</label><input type="date" value={data} onChange={e=>setData(e.target.value)}/></div></div><label>Podpis odbiorcy</label><input value={podpis} onChange={e=>setPodpis(e.target.value)} placeholder="Imię i nazwisko"/><div className="row"><button className="big blue" onClick={()=>save('Wydanie')}>Wydanie</button><button className="big green" onClick={()=>save('Zwrot (PZ)')}>Zwrot (PZ)</button></div></section><section className="card"><h2>Ostatnie operacje</h2><Table rows={p.operacje.slice(-8).reverse()} cols={['data_operacji','typ','kontrahent','opakowanie','ilosc','magazyn']}/></section></div>}
function Kontrahenci({kontrahenci,load}){const [n,setN]=useState(''),[g,setG]=useState(''),[lim,setLim]=useState(0),[sal,setSal]=useState(0),[q,setQ]=useState(''); const add=async()=>{if(!n)return; const r=await supabase.from('kontrahenci').insert({nazwa:n,grupa:g,limit_opakowan:Number(lim)||0,saldo_startowe:Number(sal)||0}); if(r.error) alert(r.error.message); setN('');load()}; const upd=async(id,o)=>{await supabase.from('kontrahenci').update(o).eq('id',id);load()}; return <section className="card"><h2><Users/> Kontrahenci</h2><div className="formline"><input placeholder="Nazwa" value={n} onChange={e=>setN(e.target.value)}/><input placeholder="Grupa" value={g} onChange={e=>setG(e.target.value)}/><input placeholder="Limit" type="number" value={lim} onChange={e=>setLim(e.target.value)}/><input placeholder="Saldo startowe" type="number" value={sal} onChange={e=>setSal(e.target.value)}/><button onClick={add}>Dodaj</button></div><input placeholder="Szukaj kontrahenta" value={q} onChange={e=>setQ(e.target.value)}/><List data={kontrahenci.filter(x=>norm(x.nazwa).includes(norm(q)))} render={x=><><b>{x.nazwa}</b><span>{x.grupa||''}</span><Actions x={x} upd={upd}/></>}/></section>}
...
function Simple({title,icon,table,rows,name,load,active,hidden}){const [n,setN]=useState(''); const add=async()=>{if(!n)return; await supabase.from(table).insert({[name]:n}); setN('');load()}; const upd=async(id,o)=>{await supabase.from(table).update(o).eq('id',id);load()}; return <section className="card"><h2>{icon} {title}</h2><div className="formline"><input placeholder="Nowa nazwa" value={n} onChange={e=>setN(e.target.value)}/><button onClick={add}>Dodaj</button></div><List data={rows} render={x=><><b>{x[name]}</b><span>{x[active]===false?'Nieaktywny':'Aktywny'} {x[hidden]?'· Ukryty':''}</span><button onClick={()=>upd(x.id,{[active]:!x[active]})}>{x[active]===false?'Aktywuj':'Dezaktywuj'}</button><button onClick={()=>upd(x.id,{[hidden]:!x[hidden]})}>{x[hidden]?'Pokaż':'Ukryj'}</button></>}/></section>}
function Uzytkownicy({users,magazyny,load}){const [u,setU]=useState({imie:'',telefon:'',pin:'',rola:'kierowca',magazyn:''}); const add=async()=>{if(!u.imie||!u.telefon||!u.pin)return; const r=await supabase.from('uzytkownicy').insert(u); if(r.error) alert(r.error.message); setU({imie:'',telefon:'',pin:'',rola:'kierowca',magazyn:''}); load()}; const upd=async(id,o)=>{await supabase.from('uzytkownicy').update(o).eq('id',id);load()}; return <section className="card"><h2>Użytkownicy</h2><div className="formline"><input placeholder="Imię" value={u.imie} onChange={e=>setU({...u,imie:e.target.value})}/><input placeholder="Telefon" value={u.telefon} onChange={e=>setU({...u,telefon:e.target.value})}/><input placeholder="PIN" value={u.pin} onChange={e=>setU({...u,pin:e.target.value})}/><select value={u.rola} onChange={e=>setU({...u,rola:e.target.value})}><option>admin</option><option>magazynier</option><option>kierowca</option></select><select value={u.magazyn||''} onChange={e=>setU({...u,magazyn:e.target.value})}><option value="">Bez przypisania</option>{magazyny.map(m=><option key={m.id}>{m.nazwa}</option>)}</select><button onClick={add}>Dodaj</button></div><List data={users.filter(x=>!x.ukryty)} render={x=><><b>{x.imie}</b><span>{x.telefon} · {x.rola} · {x.magazyn||'wszystkie'}</span><button onClick={()=>upd(x.id,{aktywny:!x.aktywny})}>{x.aktywny?'Dezaktywuj':'Aktywuj'}</button><button onClick={()=>upd(x.id,{ukryty:!x.ukryty})}>{x.ukryty?'Pokaż':'Ukryj'}</button></>}/></section>}
